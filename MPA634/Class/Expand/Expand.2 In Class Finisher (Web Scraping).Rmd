---
title: "Expand.2 In Class Finisher (Web Scraping)"
author: "Your Name"
date: "3/29/2022"
...
A: You can scrape everything, but it wants you to put in a ten second delay. 

Q: What can you NOT scrape off of Yelp?  
A: There are a lot of bots and pages that are not allowed. Check the "disallow" end points. 
...
```{r}
# Submit form
response <- html_form_submit(selection)
```
...
data_sci <- paste(str_to_lower(str_replace_all(course_long[22], c(" " = "-", "-for" = ""))))

# Create the correct url
course_url <- paste0("https://gradstudies.byu.edu/course/", data_sci)
```

Now we are ready to use this as the new url. 
```{r}
# Read the html
course_info <- read_html(course_url)
```
...
```{r}
request <- GET("https://api.agify.io?name=laurel")
(data <- fromJSON(rawToChar(request$content)))
...
```{r}
url <- "https://dining.byu.edu/"
locations <- read_html(url)

# Long way
names <- locations %>% 
  html_nodes(".list-items-10 .Link") %>% 
  html_text() 

names_clean <- vector("character")

for(i in seq_along(names)){
  if(names[i] != "" & names[i] != "Retail Locations" & names[i] != "Other Areas") {
    names_clean[i] <- names[i]
  }
}

names_clean <- na.omit(names_clean)

# OR
names <- locations %>% 
  html_nodes(".promo-title .Link") %>% 
  html_text() %>% 
  .[1:10]
...
```{r}
url <- "https://dining.byu.edu/creameries"

creamery_names <- read_html(url) %>% 
  html_nodes(".promo-title .Link") %>% 
  html_text()
...
urls <- creamery_names %>% 
  str_to_lower() %>% 
  str_replace_all(" ", "-") %>% 
  paste0("https://dining.byu.edu/", .)

address <- vector("character", length = length(urls))
phone_number <- vector("character", length = length(urls))

creamery_df <- data.frame(creamery_names, address, phone_number)
for(i in seq_along(urls)){
  url <- bow(urls[i])
  cream_info <- scrape(url)
  creamery_df[i, 2] <- cream_info %>% 
    html_nodes("#richtext-contact+ .container-fixed p+ p") %>% 
    html_text() %>% 
    str_trim() %>% 
    substr(1, nchar(.) - 12) %>% 
    str_trim()
  creamery_df[i, 3] <- cream_info %>% 
    html_nodes("#richtext-contact+ .container-fixed p+ p") %>% 
    html_text() %>% 
    str_trim() %>% 
    substr(nchar(.) - 12 + 1, nchar(.))
}
```
