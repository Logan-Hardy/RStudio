---
title: "Explore.3 In Class Starter (Description)"
...
#unique(crash_2019$COUNTY_NAME) %>% SORT()
...
```{r}
length(unique(crash_2019$COUNTY_NAME))
```
...
median(crash_2019$MILEPOINT, na.rm=TRUE)
sd(crash_2019$MILEPOINT, na.rm=TRUE)
```
...
quantile(crash_2019$MILEPOINT, na.rm=TRUE)["50%"]
...
```{r}
quantile(crash_2019$MILEPOINT, na.rm=TRUE)[1]
...
kurtosis(fatal_police_shootings$age, na.rm=TRUE)
skewness(fatal_police_shootings$age, na.rm=TRUE)
