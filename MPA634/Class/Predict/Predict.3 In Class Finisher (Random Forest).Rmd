...
```{r}
CAR_DETAILS_FROM_CAR_DEKHO <- read_csv("Datasets for playing/Predictive Analytics/CAR DETAILS FROM CAR DEKHO.csv") %>% mutate_if(is.character, as.factor)
```
...
A: It was about 29% error, so about 71% accurate.
...
```{r}
(model <- randomForest(owner ~ ., data=train, ntree = 500, mtry = 3, proximity=TRUE))
```
...
A: 69.5, just below the accuracy for the trained model. 

Q: Why might you care about the percent of positively and negatively predicted values?  
A: There may be times when it is more important to be correct on guessing things are a certain category than things aren't. For example, in a medical setting, you may want to avoid a false positive diagnosis at all costs since that could lead to unnecessary medical procedures. On the other hand, maybe you want to avoid a false negative response because you might miss the opportunity to treat someone that really needs to be treated. Depending on your goals you can then tweak the model to be better at avoiding one or the other. 
...
A: Second owners and below are more likely to have earlier years, lower prices, and higher kilometers driven. There isn't a string relationship between the categorical variables. 
...
A: Automatic cars are selling for higher prices. There is a slight bit more relationship between categories here. 
