---
title: "Predict.2 In Class Finisher (MLR in R)"
...
```{r}
fct_count(covid_clean$mask_requirement2020, sort = TRUE)
```
...
```{r}
model <- lm(cases_per_cap~.,covid_clean) 
summary(model)
```
...
A: We will point out all of these in class. 
...
```{r}
model2 <- lm(cases_per_cap~POPESTIMATE2020 + perc_age_0_19 + pov_perc,covid_clean) 
summary(model2) 
```
...
# Modeling 
This is all fine and dandy, but to really get into the data science process we have to make sure we have the best model. Right now our R-Squared is 0.2197644. Can we beat that?
...
A: Overall the highest R-squared is definitely for knn, but they are also showing moderate levels of error compared to some of the other models. Nonetheless, let's use our impute_knn dataset from now on to see if we can improve this model. 
...
log_model <- lm_fun(knn_log_y, "y_log", results = FALSE)
...
predict(log_model, newdata = county_test)
```

Before you go jumping to the conclusion that these are the cases predictions, remember these are the logarithmic version of our correct predictions. To get our actual predictions we need to exponentiate these numbers to reverse the logarithm.
```{r}
...
A: These are the predictions based on county information of what percentage of the population would have already had covid by February 2022 when the covid case numbers came through. We could always also multiply these numbers by each county's population to figure out how many estimated cases they have. 
