---
title: "Clean.4 In Class Starter (Reshaping)"
author: "Your Name"
date: "2/4/2022"
...
load("../Data/reshaping_data.RData")
```
...
ave_ten_yr_home_price_ut_counties %>% 
  pivot_longer(cols = c("1996-04":"2015-04"), 
               names_to = "yearmo", 
...
```{r}
left_join(first_camp_attended %>% select(Patient_ID, Donation),
patient_profile %>% select(Patient_ID, Income, Education_Score), by = c("Patient_ID" = "Patient_ID"))
...
There is one more join function that we sometimes need. Anti_join keeps only obsevations in one dataset that aren't in the other. Take a look: 
```{r}
...
## rbind 
rbind() works the same way except it requires matching column names. To do this successfully you need to make sure that the *columns* align in each dataset you're binding.
