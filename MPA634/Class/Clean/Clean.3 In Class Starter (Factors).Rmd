---
title: "Clean.3 In Class Starter (Factors)"
author: "Your Name"
date: "2/1/2022"
...
```{r}
mental_health <- read_csv("../Data/mental_health_tech_survey.csv") %>% mutate_if(is.character, as.factor)
```
...
```{r}
# Make this variable into only lowercase. This will coerce it to a character vector, so coerce it back to factor.
...
mental_health_fct <- mental_health_fct %>% mutate(Gender = as.factor(str_replace_all(Gender, c("(cis)" = "",                                                                               "cis " = "", 
  "cis-" = "",
  "cis" = "",
  "make" = "male",
  "maile" = "male",
...
mental_health_fct <- mental_health_fct %>% mutate(Gender = fct_collapse(mental_health_fct$Gender, 
             female = female, 
             male = male, 
             other = other))
...
# Now coerce logical and ordered into numerical columns. You may not realize, but you know this code well already! We use it almost every time we import. 
mental_health_num <- mental_health_num %>% mutate_if(is.logical, as.numeric) %>% 
...
```{r}
fatal_police_shootings <- read_csv("") %>% mutate_if(is.character, as.factor)
```
