---
title: "Clean.2 In Class Finisher (Strings)"
author: "Your Name"
date: "1/29/2022"
...
```{r}
covid19_tweets <- read_csv("~/Desktop/R Working Directory/Datasets for playing/Webscraped/covid19_tweets.csv")
```
...
A: One of two things: 1) Try to put text in a useable format, 2) generate features (variables) for analysis.  
...
covid19_tweets_edit <- 
  covid19_tweets_edit %>% mutate(nchar_tweet = nchar(text))
```
...
```{r}
covid19_tweets_edit <- 
  covid19_tweets_edit %>% mutate(text = str_to_lower(text))
```
...
covid19_tweets_edit <- 
  covid19_tweets_edit %>% 
  mutate(hashtag_count = str_count(hashtags, "#"))
```
...
Make a new new copy
```{r}
# Make a copy of the copied dataset that has 1) a unique identifier (or a combo of identifiers) and 2) the text column. 
sentiment_tweets <- covid19_tweets_edit %>% 
                          select(user_name, date, text)
```
...
Now we are going to remove all stopwords, meaning common words that do not contribute greatly to the meaning or emotion of a text. 
```{r}
...
```{r}
# Adjust the following code to merge the emotion dictionary with our sentiment dataset. (All you have to change is the name of the dataset when it appears and the unique identifier!) 
sentiment_tweets <- sentiment_tweets %>% 
...
A: positivity_binary has words characterized as only positive or negative. The positivity_weights gives a weight to how positive or negative those words are. So a very positive or very negative word will influence the overall score more in the positivity_weights column than the positivity_binary column. The positive just has a count of the number of positive words.  
...
covid19_tweets_edit %>% 
        ggplot(aes(y = nchar_tweet, x = anger)) + 
...
```
You try to make a visualization from this data using the visualization rmd
```{r}
