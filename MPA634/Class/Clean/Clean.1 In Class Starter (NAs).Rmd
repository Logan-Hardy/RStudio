---
title: "Clean.1 In Class Starter (NAs)"
author: "Your Name"
date: "1/28/2022"
...
```{r}
np_survey <- read_csv("../Data/np_survey.csv", na = c(NA, "", " ", "?") %>% mutate_if(is.character, as.factor)
```
...
(np_survey_reduced <- np_survey_reduced[rowSums(is.na(np_survey_reduced)) != ncol(np_survey_reduced),])
```
...
```{r}
np_survey_reduced <- np_survey_reduced[-82,]
...
There is one more method we can use to import missing values. The VIM packages has a function called kNN that can be used to impute missing values. This is an algorithm that tries to figure out the best value based on other data points that are around the missing point. 
```{r}
...
# write.csv(np_survey_reduced, "../Data/np_survey_reduced.csv", row.names = FALSE)
