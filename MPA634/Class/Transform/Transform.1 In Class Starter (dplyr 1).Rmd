---
title: "Transform.1 In Class Starter (Dplyr 1)"
author: "Logan Hardy"
date: "01/18/2022"
...
refugees2011_15 %>% select(Year, Country, AsylumSeekers)
...
refugees2011_15 %>% select(!Country)

# Put the TotalPop column first and have everything the same after. 
refugees2011_15 %>% select(TotalPop, everything())



# Grab all the columns that start with "C"
refugees2011_15 %>% select(starts_with("c"))


# Grab all the numeric columns
refugees2011_15 %>% select(is.numeric)
...
refugees2011_15 %>% filter(Refugees > AsylumSeekers)
...
```{r}
# Make a subset that shows just data where asylum seekers are greater than 10,000 and the country of asylum is Belgium. The Asylum seeker column should be renamed "Num_Asylum_Seekers" 
refugees2011_15 %>% 
...
refugees2011_15 %>% mutate(Country = as.character(Country))
...
refugees2011_15 %>% mutate(char_country = as.character(Country))
...
refugees2011_15 %>% mutate_if(is.factor, as.character)
...
refugees2011_15 %>% mutate(Year = as.integer(Year), Origin = as.character(Origin))
...
refugees2011_15 %>% mutate(Origin = str_to_lower(Origin))
...
refugees2011_15 %>% mutate(pop_less_asy_and_ref = TotalPop - Refugees - AsylumSeekers)

# Make a new column called "RefugeesSquared" that squares the number of refugees. 
refugees2011_15 %>% mutate(RefugeesSquared = Refugees ^ 2)
...
refugees2011_15 %>% mutate(ref_greater = ifelse(Refugees > AsylumSeekers, TRUE, FALSE))
...
refugees2011_15 %>% mutate(countries = ifelse(as.character(Origin) == as.character(Country), "SAME", "Different"))
...
refugees2011_15 %>% na.omit(Refugees)
...
refugees2011_15 %>% select(Year, Country, Origin, AsylumSeekers) %>% na.omit()
...
refugees2011_15 %>% filter(!is.na(Refugees))

# Now your turn. Filter out just NAs in the TotalPop column. 
refugees2011_15 %>% filter(!is.na(TotalPop))
