---
title: "Transform.3 In Class Finisher (Visualize)"
...
A: A column chart accepts 2 variables, a numeric and categorical. They need a one to one ratio. A bar chart accepts one categorical variable, and fills bars by the number of times the categories are in the dataset. 
...
gapminder %>% 
  filter(continent == "Americas") %>% 
        ggplot(aes(y = country, x = lifeExp, fill = country)) + 
        geom_col() +
        labs(title = "Title", 
            subtitle = "Subtitle") +
        theme_new + 
...
A: About 775. It has added up all the numeric variable together. 
...
gapminder %>% 
  filter(continent == "Americas", 
         year == 2007) %>% 
        ggplot(aes(y = fct_reorder(country, lifeExp), x = lifeExp, fill = country)) + 
        geom_col() +
        labs(title = "Title", 
            subtitle = "Subtitle") +
        theme_new + 
...
gapminder %>% 
  ...
  summarize(median_life = median(lifeExp)) %>% 
        ggplot(aes(y = country, x = median_life, fill = country)) + 
        geom_col() +
        labs(title = "Title", 
            subtitle = "Subtitle") +
        theme_new + 
...
gapminder %>% 
  ...
  group_by(country) %>% 
  summarize(median_life = median(lifeExp)) %>% 
        ggplot(aes(y = fct_reorder(country, median_life), x = median_life, fill = country)) + 
        geom_col() +
        labs(title = "Title", 
            subtitle = "Subtitle") +
        theme_new + 
...
gapminder %>% 
  filter(pop <= 2000000, 
         continent != "Oceania") %>% 
        ggplot(aes(x = fct_reorder(continent, pop, median, na.rm = TRUE), y = pop, fill = continent)) + 
...
gapminder %>% 
  filter(pop <= 2000000, 
        ...
        ggplot(aes(x = pop, fill = continent, color = continent)) + 
        geom_density(alpha = .3) +
        labs(title = "Density plot of the populations by continent, 1952-2007", 
            subtitle = "Europe has the clearest bimodal distribution", 
            x = "Population", 
...
gapminder %>%  
        ...
        labs(title = "Life expectancy distribution for continents", 
            subtitle = "Overall, European countries have had the highest life expectancy between 1952-2007", 
            caption = "Source: Gapminder Dataset") + 
        theme_new
...
gapminder %>% 
        ...
        labs(title = "Life expectancy change 1952-2007 for countries in the Americas", 
          subtitle = "The US has seen one of the largest increases in life expectancy", 
          x = "x variable", 
          y = "y variable", 
          caption = "Source: Gapminder Dataset") +
        coord_flip() + 
...
# This is part of the graph in every case. 
      ...
      labs(title = "Per Capita GDP in Bangladesh 1952-2007", 
          subtitle = "There was a significant increase between 2002 and 2007",
          y = " Per Cap GDP in Dollars",
          x = "", 
          caption = "Source: Gapminder Dataset") +
