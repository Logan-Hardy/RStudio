---
title: "Transform.5 In Class Finisher (Functions)"
...
A: They need to be vectors, numeric, logical, date, date-time, and time interval. 
...
## Methods
```{r}
# Run the following code. This shows us the source code of the mean function. 
getAnywhere(mean.default)
```

Q: Using the source code, what package is this function from? 
A: Base

Q: What happens if someone tries to put a non-numeric value in for "trim"?
A: It will return the error 'trim' must be numeric of length one"
...
are_animals <- function (vec, str_to_title = TRUE) {
  for (animal in vec) {
    if (str_to_title){
      print(paste0(str_to_title(animal), " are animals."))
    } else {
      print(paste0(animal, " are animals."))
    }
  }
...
are_animals(birds, str_to_title = FALSE)
```

```{r}
# Your turn! Write a function that accepts a vector and prints each element in the vector with the number of characters in the element. So if you run the birds vector into your function, it should output "parrots has 7 letters." "chickadees has ten letters." "finches has seven letters." 

# Hint: the function nchar() counts the number of characters in a string. 
count_char <- function (vec) {
  for (v in vec) {
    print(paste0(v, " has ", nchar(v), " letters."))
  }
}
...
count_char <- function (vec, numbers = FALSE) {
  if (numbers) {
    for (v in vec) {
      print(paste0(v, " has ", nchar(v), " numbers."))
    }
  } else {
    for (v in vec) {
      print(paste0(v, " has ", nchar(v), " letters."))
...
A: The looped through printed sentences. 
...
are_animals <- function (vec, str_to_title = TRUE) {
  for (animal in vec) {
    if (str_to_title){
      return(print(paste0(str_to_title(animal), " are animals.")))
    } else {
      return(print(paste0(animal, " are animals.")))
    }
  }
}
output <- are_animals(animals)
...
```{r}
# Create a function that prints the mean, median, and standard deviation of a numeric vector. Include a parameter for na.rm. Output the error "This is not a numeric vector" if a non-numeric vector is input into the function. 
descr_stats <- function (vec, na.rm = TRUE) {
  if (!is.numeric(vec)) stop("This is not a numeric vector") 
...
# Practice 
```{r}
# Create a function that accepts a dataframe and prints the number of missing values in each column in sentence format. "Column 1 has x NAs."
count_nas <- function(df){
  for (i in seq_along(df)) {
    print(paste0("Column ", i, " has ", sum(is.na(df[[i]])), " NAs."))
  }
}
...
count_nas <- function(df){
  if (!is.data.frame(df)) {stop ("Input is not a dataframe") }
  for (i in seq_along(df)) {
    print(paste0("Column ", i, " has ", sum(is.na(df[[i]])), " NAs."))
  }
}
...
count_nas <- function(df, name = TRUE){
  if (!is.data.frame(df)) {stop ("Input is not a dataframe") }
  if (name) {
    for (i in seq_along(df)) {
      print(paste0("The ", colnames(df)[[i]], " column has ", sum(is.na(df[[i]])), " NAs."))
    }                    
  } else {
    for (i in seq_along(df)) {
      print(paste0("Column ", i, " has ", sum(is.na(df[[i]])), " NAs."))
