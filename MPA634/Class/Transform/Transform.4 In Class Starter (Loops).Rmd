---
title: "Transform.4 In Class Starter (Loops)"
...
for (i in seq_along(mtcars1)) {
  for (j in seq_along(mtcars1)) {
    mtcars1[[i]][[j]] <- trunc(round(mtcars[[i]][[j]], digits = 0))
...
for (col in colnames(mpg)){
   if (class(mpg[[col]]) %in% c("numeric", "integer")){
